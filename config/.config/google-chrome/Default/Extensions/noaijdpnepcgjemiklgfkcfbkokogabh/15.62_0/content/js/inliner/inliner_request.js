...
function injectScript(id, url, param, dictionary, text, langDst) {
 try{
	...
	if(LISTofLANGsets[1].indexOf(langDst)==-1) ACT_PR="Google";
			    ...
			    if(AUTO=="auto"){
				    ...
				    setTimeout(function() {
						...
						if(DetLang!="") {
							...
							if(LISTofLANGsets[1].indexOf(ln)==-1) SL_OTHER_PROVIDERS(text, ln, 0);
...
function SL_OTHER_PROVIDERS(text,ln,st){
	...
	if(LOCAL_FILES==1) st=0;
        ...
        else cgi = "dir="+DetLang+"/"+ln+"&provider=microsoft&text="+encodeURIComponent(text);
	var ajaxRequest;  
	try{
		ajaxRequest = new XMLHttpRequest();
	} catch (e){
...
function injectScriptRear(id, dictionary, text, langDst) {
    var xhr = getHttpRequest();
    var url = ImTranslator_theurl+"dotrans.php";
...
// By VK ------ Translation goes without the detection engine-------------
	...
	baseUrl = "https://translate.google."+theRegion;
        baseUrl = baseUrl+"/translate_a/t";
	// By VK ------ Translation goes without the detection engine-------------
        SL_Params = "client=dict-chrome-ex&sl=auto&tl=" + langDst + "&multires=1&otf=2&ssel=0&tsel=0&notlr=0&sc=1&tk=" + tk + "&text=" + encodeURIComponent(dictionaryWord);
	// By VK ------ Translation goes without the detection engine-------------
...
//        SL_Params = "client=dict-chrome-ex&hl=en&langpair=auto|" + langDst + "&q=" + encodeURIComponent(escapedText) + "&tbb=1&ie=UTF-8&oe=UTF-8";
//        SL_Params = "client=dict-chrome-ex&hl=en&sl=auto&tl=" + langDst + "&q=" + encodeURIComponent(escapedText) + "&tbb=1&ie=UTF-8&oe=UTF-8";
