...
setTimeout(function() {
 (function(){if(GEBI('SL_save_button2'))GEBI('SL_save_button2').addEventListener("click",function(){SL_SAVE();},!1);} )();
}, 1000);
...
function SL_LTR_RTL(dir){
 var TheClass="SL_ltr";
 if(dir=="ar" || dir=="iw" || dir=="fa" || dir=="ur" || dir=="yi" || dir=="ps" || dir=="sd") var TheClass="SL_rtl";
 return(TheClass);
}
...
//       if(TMP[3].indexOf("chrome-extension://")!=-1 && TMP[3].split("/").length<=2) TMP[3] = "undefined";
...
function SL_TRANS(text,dir,ind1,ind2,provider){
  switch(provider){
		...
		var SL_Params = "client=dict-chrome-ex&hl=en&sl=auto&tl="+TMP_DIR[1]+"&q="+encodeURIComponent(text)+"&tbb=1&ie=UTF-8&oe=UTF-8";
...
//  if(dir=="uk") client="t";
  ...
  if(!document.getElementById("PL_lbframe")){
    ...
    if(document.getElementById('SL_Hplayer'+CurTTSIND)) document.getElementById('SL_Hplayer'+CurTTSIND).appendChild(die);
     ...
     audioElement.setAttribute('style', 'width:680px;margin-top:-15px;margin-left:-20px;');
...
function SL_ALIGNER(SRC,L1,L2){
 var OUT="";
 if(L2=="ar" || L2=="iw" || L2=="fa" || L2=="ur" || L2=="yi" || L2=="ps" || L2=="sd"){
 ...
 if(L1=="ar" || L1=="iw" || L1=="fa" || L1=="ur" || L1=="yi" || L1=="ps" || L1=="sd")   OUT = OUT.replace(/ id=_AR>/g, ' id=_AR style="text-align:right">');
		...
		var cgi = "dir="+f+"/"+t+"&provider="+provider.toLowerCase()+"&text="+encodeURIComponent(text);
			...
			ajaxRequest.onreadystatechange = function(){
				...
				if(ajaxRequest.readyState == 4){
			             ...
			             if(resp.indexOf('<h1>Url Too Long</h1>')!=-1 || resp.indexOf('<TITLE>Request URL Too Long</TITLE>')!=-1 || resp.indexOf('"ArgumentOutOfRangeException:')!=-1) resp=SL_LOC("lim2000").replace("XXX","4000");
	                             resp=resp.replace(/\\n/ig,"<br>");
	                             resp=resp.replace(/\n/ig,"<br>");
