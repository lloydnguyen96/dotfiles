				...
				if(TranslatorIM.SL_ENABLE == "true"){
					...
					if(target.className.toString().indexOf("CodeMirror-")==-1){
						TranslatorIM.SL_LNG_LIST = TranslatorIM.CUSTOM_LANGS(FExtension.element(TranslatorIM.SL_LOC,'extLanguages'));	       
					        if(e.which != 3){
							//WPT------
							if(id.indexOf("SL_")==-1 && id=="SL_WPT_SHOW") TranslatorIM.SL_WPT_OPTIONS_CLOSE();
							//WPT------
			...
			if(ROUT==1) TranslatorIM.SL_HideButton();
				...
				if(TranslatorIM.SL_ENABLE == "true"){
					if(FExtension.browserInject.getSelectionText()!=""){
						...
						try{
							if(TranslatorIM.SL_TS!=TranslatorIM.SL_TSold) TranslatorIM.SL_OnOff_BTN_SID = "";
							if(TranslatorIM.SL_OnOff_BTN_SID=="") TranslatorIM.SL_OnOff_BTN_SID = TranslatorIM.SL_OnOff_BTN;
						...
						if(wl.indexOf("http")!=-1){
							if(TranslatorIM.SL_SAVETEXT == 1){
				  	       ...
				  	       if(TranslatorIM.SL_OnOff_BTN_SID=="true") {
						        var ext = FExtension.browserInject;
						  	doc.getElementById('SL_button').style.background='url('+ext.getURL('content/img/util/imtranslator-s.png')+')';
							doc.getElementById('SL_button').style.opacity=100;
	...
	keyup:function(e){ 
		...
		if(TranslatorIM.SL_MSG.indexOf("~")!=-1){
			...
			if(TranslatorIM.SL_OPTbox == "true"){
	                        if(TranslatorIM.SL_SYNC_FILTER(DBHOTKEYSlineOPT,HOTKEYSline)==true) window.open(FExtension.browserInject.getURL('options.html', true),"ImTranslator:Option");
	                }
	...
	QuickInitTranslators: function(e){
		...
		setTimeout(function() {
		  try{
			...
			if(theSLtext!=""){
			   	if(TranslatorIM.SL_OnOff_BTN_SID=="true" && theSLtext.indexOf("Google - Map Data")==-1 && theSLtext!="." && theSLtext!="," && theSLtext!="?" && theSLtext!="!" && theSLtext!=":" && theSLtext!=";" && theSLtext!="-"){
				 	if(TranslatorIM.SL_ENABLE=="true"){
...
/*
	SL_HideButton__: function(){
		...
		FExtension.browserInject.getDocument().getElementById("SL_button").style="none";
...
*/
	...
	SL_GetButtonCurPosition: function (X1,Y1,X2,Y2) {
		...
		var bodyScrollTop = doc.documentElement.scrollTop || doc.body.scrollTop;
        	...
        	if(((Y2+delta1+AT)+correctionY)<=bodyScrollTop)	SLdivField.style.top = (bodyScrollTop+20)+"px";
        	if(((Y2+delta1+AT)+correctionY)>=(bodyScrollTop+window.innerHeight-40))	SLdivField.style.top = (bodyScrollTop+window.innerHeight-40)+"px";
	...
	SL_ShowBalloon: function(){
		...
		if(doc.body.innerHTML.indexOf("/1999/xhtml")!=-1) {	
			//runinliner();
			var theSLtext = FExtension.browserInject.getSelectionText();
		...
		if(SLdivField.style.backgroundColor==""){
			...
			if(theSLtext != ""){
				...
				TranslatorIM.SL_BALLOON_TRANSLATION(theSLtext,evt,0, win);
						...
						case "TOP":  	SLdivField.style.top=TranslatorIM.SL_T+bodyScrollTop-277+"px"; 
...
//			    	TranslatorIM.SL_addEvent(SL_BBL_locer, 'click', TranslatorIM.SL_BBL_locer);  
		...
		if(TranslatorIM.SL_FONT2!=TranslatorIM.SL_FONT || TranslatorIM.BBL_TS!=TranslatorIM.BBL_TS2)TranslatorIM.SL_FONT_SID="";

		if(TranslatorIM.SL_FONT_SID==""){
		 	TranslatorIM.SL_FontSize = TranslatorIM.SL_FONT;
			TranslatorIM.SL_FONT2 = TranslatorIM.SL_FONT;
			TranslatorIM.BBL_TS2 = TranslatorIM.BBL_TS;
		}else TranslatorIM.SL_FontSize = TranslatorIM.SL_FONT_SID;
		...
		if(TranslatorIM.SL_FontSize == "14px")   OBJ3.className="SL_font_on";
		if(TranslatorIM.SL_FontSize == "16px")   OBJ3.className="SL_font_off";
		// COPY icon
	...
	SL_BBL_locer_settler: function(){
	 ...
	 if(TranslatorIM.SL_OnOff_BTN_SID=="true") doc.getElementById('SL_BBL_locer').checked=true;
		...
		if(TranslatorIM.SL_EVENT){ 
			var ev = TranslatorIM.SL_EVENT;
				...
				if(doc.getElementById('SL_BBL_locer')) {
					if(ev.target.id == "SL_BBL_locer") TranslatorIM.SL_OnOff_BTN_SID = doc.getElementById('SL_BBL_locer').checked.toString();
				}
			}
		}
	},
	...
	SL_locker_settler: function(){
	        var doc = FExtension.browserInject.getDocument();
	        if(TranslatorIM.SL_TS!=TranslatorIM.SL_TSold){
			if(doc.getElementById('SL_lng_from').value=="auto") TranslatorIM.SL_NODETECT_bbl="true";
			if(TranslatorIM.SL_NODETECT_bbl=="true"){
	...
	SL_bring_UP: function(){
	    try{
		if(window.event && window.event.which!=3){
			...
			if(ToLng=="ar" || ToLng=="iw" || ToLng=="fa" || ToLng=="ur" || ToLng=="yi" || ToLng=="ps" || ToLng=="sd"){
...
//	    if(TranslatorIM.SL_OnOff_BTN_SID=="true")   	doc.getElementById('SL_button').style.background='url('+ext.getURL('content/img/util/imtranslator-s.png')+')';
						...
						if(TranslatorIM.BBL_DETECT!="") {
							...
							if(doc.getElementById("SL_shadow_translator").style.display=='block'){	
								...
								if(resp.indexOf('ld_result":{"srclangs":["')!=-1) {
									...
									if(DetLang=="zh-CN"){ TranslatorIM.SLDetector(myTransText); return false;}
									...
									if(TranslatorIM.AutoFlipState==1){
//										doc.getElementById('SL_lng_from').value=TranslatorIM.SL_langSrc;
//										doc.getElementById('SL_lng_to').value=TranslatorIM.SL_langDst;
									}
	...
	SL_Voice: function(){
           var doc = FExtension.browserInject.getDocument();
	   doc.getElementById('SL_alert100').style.display="none";
	   var SL_from = TranslatorIM.SL_DETECT;
...
//	  if(BackUpDir=="es") client="t";
	  ...
	  if(!doc.getElementById("lbframe")){
	    ...
	    doc.getElementById('SL_player2').appendChild(die);
	     ...
	     doc.getElementById('SL_player2').style.width="461px";
	...
	SL_Font: function(){
		...
		var OBJ3 = doc.getElementById('SL_bbl_font');
		if(TranslatorIM.SL_FontSize == OBJ.style.fontSize){
			if(TranslatorIM.SL_FontSize=="14px"){
				OBJ.style.fontSize = "16px";
				OBJ.style.lineHeight = "22px";
				OBJ2.style.fontSize = "16px";
				OBJ2.style.lineHeight = "22px";
				...
				TranslatorIM.SL_FontSize = "14px";
			}
			TranslatorIM.SL_FONT_SID = TranslatorIM.SL_FontSize;
		}
                TranslatorIM.SL_JUMP(doc);
...
//			        if(TranslatorIM.SL_dict_bbl=="false") theQ=100;
	...
	//			if(Stemp=="en" || TranslatorIM.SL_FLAG==0) Stemp="auto";
				...
				if(theQ==1){
					...
					myTransText=myTransText.replace(/\(/gi,"");
					myTransText=myTransText.replace(/\"/gi,"");
					SL_Params = "client=dict-chrome-ex&sl="+Stemp+"&tl="+T+"&q=" + encodeURIComponent(myTransText) + "&tbb=1&ie=UTF-8&oe=UTF-8&tk="+tk;
				}
...
//										resp = resp.replace(/\"/ig,"'");
								...
								if(T == "ar" || T == "iw" || T == "fa" || T == "ur" || T == "yi" || T == "ps" || T == "sd"){
...
//	        if(S=="auto" && TranslatorIM.SL_DETECT != "") S=TranslatorIM.SL_DETECT;
		...
		var PR = TranslatorIM.BL_T_PROV;
		if(TranslatorIM.SL_MODE==1) PR = TranslatorIM.BL_D_PROV;
	        if(PR.toLowerCase()=="yandex") { TranslatorIM.SL_YANDEX(text,S,T); return false;}
		if(text != ""){
			...
			if(S=="auto")S=TranslatorIM.LNGforHISTORY;
				...
				T = T.replace("zh-CN", "zh-CHS");
			        T = T.replace("zh-TW", "zh-CHT");
			        T = T.replace("iw", "he");
			        T = T.replace("bs","bs-Latn");
			        T = T.replace("tl","fil");
			        T = T.replace("hmn","mww");
			        T = T.replace("sr","sr-Cyrl");
			...
			ajaxRequest.onreadystatechange = function(){
				...
				if(ajaxRequest.readyState == 4){
			             ...
			             if(resp.indexOf('>Url Too Long<')!=-1 || resp.indexOf('>Request URL Too Long<')!=-1 || resp.indexOf('"ArgumentOutOfRangeException')!=-1) resp=FExtension.element(TranslatorIM.SL_LOC,"extlim2000").replace("XXX","4000");
			             if(resp=="" || resp.indexOf("<h1>Not Found</h1>")>-1) resp=FExtension.element(TranslatorIM.SL_LOC,"extnotrsrv");
					...
					setTimeout(function() {	
							...
							doc.runtimeSendMessage({greeting:"[b]" + text + "~~" + resp + "~~" + LNGfrom + "|" + T + "~~"+ doc2.location+"~~"+CurDT+"~~"+HID+"~~"+DICT[0]+"^^"}, function(response) {
								if(response){ 
								//	console.log(response.farewell); 
								}
							});
				...
				for(j = 0; j < T2.length; j++){
					...
					if(R[1] != undefined){
						...
						for(k = 0; k < R2.length; k++){
							if(k < (R2.length-1))	Rline = Rline + "<a class=_ALNK title=\""+R2[k]+"\" id='SL_" +i+"_"+ j+"_"+ k + "' href='"+Gurl+"?dir="+ SL_from + "|" + SL_to +"&text=" + encodeURIComponent(R2[k]) + "'>" + R2[k] + "</a>, ";
							else			Rline = Rline + "<a class=_ALNK title=\""+R2[k]+"\" id='SL_" +i+"_"+ j+"_"+ k + "' href='"+Gurl+"?dir="+ SL_from + "|" + SL_to +"&text=" + encodeURIComponent(R2[k]) + "'>" + R2[k] + "</a>";

						}
						var SL_myTTS = article + T2[j];

						if(TranslatorIM.SL_ALLvoices.indexOf(SL_to)!=-1){
						   if(WAY2==1) SL_myTTS = "<div id=_X><div id=_XL><div class=_V id=\"SL_"+i+j+"\" style=\"background:url("+ttsurl+");\" lang=\""+SL_to+"\" title=\"" + T2[j] + "\"></div></div><div id=_XR>" + article + T2[j] + "</div></div>";
						   else SL_myTTS = "<div id=_X><div id=_FL><div class=TTS"+WAY2+" id=\"SL_"+i+j+"\" style=\"background:url("+ttsurl+");\" lang=\""+SL_to+"\" title=\"" + T2[j] + "\"></div></div><div id=_XR>" + article + T2[j] + "</div></div>";
						}				
						parsedRES = parsedRES + "<div id=_A><div id=_AL>" + SL_myTTS + "</div><div id=_AR>" + Rline + "</div></div>";
					  } else {
						if(TranslatorIM.SL_ALLvoices.indexOf(SL_to)!=-1){
						   SL_myTTS = "<div id=_X><div id=_XL><div class=_V id=\"SL_"+i+j+"\" style=\"background:url("+ttsurl+");\" lang=\""+SL_to+"\" title=\"" + T2[j] + "\"></div></div><div id=_XR>" + article + T2[j] + "</div></div>";
						}else{
...
SL_TTSicn: function (lng){
 var doc = FExtension.browserInject.getDocument();
 var OUT="";
 if(lng!="ar" && lng!="iw" && lng!="fa" && lng!="ur" && lng!="yi" && lng!="ps" && lng!="sd")  OUT=1;
 else   OUT=2;
 return(OUT);
},
	...
	ClickInside: function(event){
		 ...
		 var ahref = FExtension.browserInject.getDocument().getElementById(id).href;
		...
		if(TranslatorIM.SL_GVoices=="1"){
			if(text.length>TranslatorIM.GTTS_length){
			   ...
			   doc.getElementById('SL_alert100').style.display="block";
	        	...
	        	TranslatorIM.REMOTE_Voice(TranslatorIM.SL_DETECT,text);			
		} else window.open("https://text-to-speech.imtranslator.net/?dir="+SL_to+"&text="+encodeURIComponent(text));
	},
...
//					        			OB23.setAttributeNode(js);
		...
		var OBtbl = doc.createElement("table");
		id = doc.createAttribute("id");
		id.value = "SL_tbl_opt";
		...
		OBtbl.height = "16";
				...
				OBtd3_.align = "center";
					...
					var va = doc.createAttribute("checked");
					va.value = Boolean(TranslatorIM.SL_OnOff_BTN_SID);
				...
				OBtd2.align = "center";
					...
					hr.value = FExtension.browserInject.getURL('content/html/options/options.html?bbl');
					...
					hr = doc.createAttribute("href");
					hr.value = FExtension.browserInject.getURL('content/html/options/options.html?hist');
					...
					hr = doc.createAttribute("href");
					hr.value = FExtension.browserInject.getURL('content/html/options/options.html?feed');
...
*/
	...
	//	  if(l!=t){
			  ...
			  if((bodyScrollTop+b)>(bodyScrollTop + window.innerHeight-200) && b-t> window.innerHeight-200 && t<260) TranslatorIM.SL_NEST="FLOAT";
                          if(TranslatorIM.BBL_TS!=TranslatorIM.BBL_TS3 || TranslatorIM.BBL_TS3==""){
				TranslatorIM.SL_SID_PIN=TranslatorIM.SL_OnOff_PIN;
				TranslatorIM.BBL_TS3=TranslatorIM.BBL_TS;
			  }
			  if(TranslatorIM.SL_FRAME==0){
...
//			}
	...
	SL_WEB_PAGE_TRANSLATION: function(st, act){       
		TranslatorIM.SL_getDATA();
		setTimeout(function() {
			TranslatorIM.SL_LoadGoogleTranslateWidget(TranslatorIM.SL_LOC,TranslatorIM.SL_wptGlobLang,st,act);
		}, 500);
	},
...
/////////VK FLIP
	      ...
	      if(TranslatorIM.SL_WPTsrclang!="auto"){
		      if(TranslatorIM.SL_WPT_DET_LNG == targetLang && TranslatorIM.SL_WPT_FLAG==0){
		         	var tt = TranslatorIM.SL_WPTsrclang;
...
//////;
	...
	//         '    pageLanguage: "' + sourceLang + '", ',
	      ...
	      translateElementScript.type = "text/javascript"
              var theRegion ="com";
	      if(TranslatorIM.SL_DOM!="auto") theRegion=TranslatorIM.SL_DOM;
	      translateElementScript.src = '//translate.google.'+theRegion+'/translate_a/element.js?cb=googleTrButton' + uid + '&hl=' + sourceLang;
	      ...
	      TranslatorIM.SL_getLNG_TMP("SL_G_WPT_TO");
			...
			if(top.window.frames[":0.container"]){
		   	...
		   	if(TranslatorIM.SL_get_TIP_TRIGGER("SL_wptGlobTipTmp") != 1){
				...
				if(doc.getElementById("goog-gt-tt")) doc.getElementById("goog-gt-tt").id="goog-gt-tt_fix";
			}else{
				if(doc.getElementById("SL_fix"))head.removeChild(doc.getElementById("SL_fix"));
				var style_tip = document.createElement('style');
				style_tip.setAttribute('type', 'text/css');
				...
				style_tip.appendChild(document.createTextNode('.goog-text-highlight { background-color: transparent !important; border: none !important; box-shadow: none !important; }'));
				head.appendChild(style_tip);
				if(doc.getElementById("goog-gt-tt_fix")) doc.getElementById("goog-gt-tt").id="goog-gt-tt";
			}
...
//	var MENU = FExtension.element(TranslatorIM.SL_LOC,'extLanguages').split(",");
	...
	SL_TBc.value = "background:#ffffff;z-index:9999999999;position:absolute;text-align:right;right:35px;margin-top: -16px;";
	...
	if(SL_TB){ 
	   try{
	      ...
	      var obs =	win.document.getElementsByTagName('td');
	      for (var i=0; i<obs.length; i++){
        	  if(obs[i].className == "goog-te-banner-margin") {
			obs[i].appendChild(SL_TB);
		  }
	      }
    ...
    SL_WPT_OPTIONS_CLOSE: function(){
 	...
 	if(doc.getElementById("SL_WPT_SHOW")) doc.getElementById("SL_WPT_SHOW").style.display='none';
    },
	...
	for (var i=0; i<obs.length; i++){
 	...
 	if(doc.getElementById("goog-gt-tt")) doc.getElementById("goog-gt-tt").innerHTML="";
	 ...
	 SL_WPT_TABS.setAttributeNode(id);
	  ...
	  SL_WPT_TAB2div.onclick = function(){
		...
		SL_WPT_TAB3div.onclick = function(){
		    window.open(FExtension.browserInject.getURL('options.html?wpt', true),"ImTranslator:Option");
		};
		var id = doc.createAttribute("id");
		...
		SL_WPT_TAB4div.onclick = function(){
		    window.open(FExtension.browserInject.getURL('options.html?hist', true),"ImTranslator:History");
		};
		var id = doc.createAttribute("id");
...
//-Domain items
	...
	var tmnst = doc.createAttribute("style");
	tmnst.value = "margin-left:6px;font-size:12px !important;";
        tmn.setAttributeNode(tmnst);
	...
	//-Subdomain item
		...
		var SL_wptSDNamest = doc.createAttribute("style");
		SL_wptSDNamest.value = "margin-left:6px;";
	        SL_wptSDName.setAttributeNode(SL_wptSDNamest);
        	if(TranslatorIM.SL_wptDp2==1){
...
//		if(TranslatorIM.SL_wptDp3!="") TranslatorIM.SL_wptDp3 = TranslatorIM.SL_wptGlobLang;
		...
		for(var J=0; J < MENU.length; J++){
		    ...
		    if(v.value==TranslatorIM.SL_wptDp3){
			var s = doc.createAttribute("selected");
			s.value="true";
			option.setAttributeNode(s);
			switch(TranslatorIM.SL_wptDp3){
			  case "None": CURlang3[1]=locNone; break;
			  case "All": CURlang3[1]=locAll; break;
			  default: CURlang3[1]=TranslatorIM.SL_GetLongName(TranslatorIM.SL_wptDp3); break;
			}
	...
	//-Domain always item
		...
		var st = doc.createAttribute("style");
		st.value = "margin-left:6px;";
	        SL_Dautobox.setAttributeNode(st);
	...
	//-Domain tip item 
		...
		var st = doc.createAttribute("style");
		st.value = "margin-left:6px;";
	        SL_Dtipbox.setAttributeNode(st);
		...
		var span = doc.createElement("span");
        	...
        	SL_OPT_MENU.appendChild(span);
		//------------
		var SL_TBhr = doc.createElement("p");
		var cl = doc.createAttribute("class");
		cl.value = "SL_hr";
...
//-Language items
	//-Language item
		...
		for(var J=0; J < MENU.length; J++){
		    var CURlang3 = MENU[J].split(":");
		    var option = doc.createElement('option');
		    if(TranslatorIM.SL_wptLp1=="") TranslatorIM.SL_wptLp1 = TranslatorIM.SL_DETLANG;
		    v = doc.createAttribute("value");
		    v.value = CURlang3[0];
	...
	//-Language always item
		...
		var st = doc.createAttribute("style");
		st.value = "margin-left:6px;";
	        SL_Lautobox.setAttributeNode(st);
	...
	//-Language tip item 
		...
		var st = doc.createAttribute("style");
		st.value = "margin-left:6px;";
	        SL_Ltipbox.setAttributeNode(st);
...
//			 	case "en":LNG = "en-GB|Male"; break;
	...
	SL_YANDEX: function(text,f,t){
			...
			var baseUrl = "https://translate.yandex.net/api/v1.5/tr.json/translate?key=-theKey-&format=plain&options=1&lang="+dir+"&text="+encodeURIComponent(text); 
		        FExtension.browserInject.runtimeSendMessage({greeting: "TR_YANDEX:>"+baseUrl+","+text}, function(response) {});
			setTimeout(function(){
			    var SLIDL = setInterval(function(){
			           FExtension.browserInject.extensionSendMessage({greeting: 1}, function(response) {
			             if(response && response.farewell){
