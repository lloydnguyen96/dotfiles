ImTranslator_inliner = {
	...
	init: function(){
		...
		if (!FExtension.store.get("SL_langDst_it")) { FExtension.store.set("SL_langDst_it", "es"); }


		if (!FExtension.store.get("SL_shortcutInliner")) { FExtension.store.set("SL_shortcutInliner", "T"); }
		if (!FExtension.store.get("SL_shortcutClean")) { FExtension.store.set("SL_shortcutClean", "X"); }
	...
	translate: function (text, injectDictionary) {
		...
		var langDst = FExtension.store.get("SL_langDst_it");
	...
	respondToMessage: function(theMessageEvent) {
		if(theMessageEvent.name === "inlinerSelectionRequest") {
		...
		} else if (theMessageEvent.name === "shortcutInlinerSelectionRequest") {
		...
		} else if (theMessageEvent.name === "shortcutInlinerCleanRequest") {
			...
			FExtension.browser.inlinerPostMessage({name:"injectLangDst", message:FExtension.store.get("SL_langDst_it")});
		}
	},
