...
(function(){var loc=GEBI("SLlocpl");loc.addEventListener("click",function(){LOCcontrol(0);},!1);} )();
...
(function(){if(GEBI("SL_TR"))GEBI("SL_TR").addEventListener("click",function(){TR_WINDOW();},!1);} )();
...
function CONSTRUCTOR(){
	...
	if(BACK_VIEW==1) window.resizeTo(495,660+EX);
	else window.resizeTo(495,525+EX);
...
//  if(BackUpDir=="es") client="t";
  ...
  baseUrl = baseUrl +"?"+ SL_Params;
   ...
   if(!document.getElementById("PL_lbframe")){
	...
	setTimeout(function(){
     ...
     document.getElementById('SL_TTS_player'+box).style.height="40px";
     document.getElementById('SL_TTS_player'+box).style.width="405px";
 ...
 if(GEBI("PL_lbframe").style.display!="block"){
	...
	if(window.locationbar.visible==true)EX=25;
	if(BACK_VIEW==1){
		GEBI('SL_back').style.height='70px';
		window.resizeTo(495,(GEBI('SL_body').clientHeight-30));
	} else {
		window.resizeTo(495,(GEBI('SL_body').clientHeight+45+EX));
	}
 }	
...
function REMOTE_Voice_Close (){
 ...
 if(window.locationbar.visible==true)EX=25;
 window.resizeTo(495,(GEBI('SL_body').clientHeight+45+EX));
 if(BACK_VIEW==1) GEBI('SL_back').style.height='118px';
}
			...
			if(RespS!=RespT){
				if(comparelangs.indexOf(RespS)>-1 && comparelangs.indexOf(RespT)>-1){
					...
					for (var j=0;j<BASELANGS.length;j++){
				        ...
				        var msg=FExtension.element(FExtension.store.get("SL_LOCALIZATION"),'extNo_TextCompare').replace("XXX",RespSlong);
...
function DO_TTS_Detection(myTransText,st) {
  ...
  if(myTransText!=""){
    ...
    var theRegion = SL_GEO[num];
    if(FExtension.store.get("SL_DOM")!="auto") theRegion=FExtension.store.get("SL_DOM");
    var baseUrl = 'https://' + 'translate.google.'+theRegion+'/translate_a/single?client='+theRegion+'&sl=auto&tl=en&hl=en&dt=bd&dt=ex&dt=ld&dt=md&dt=qca&dt=rw&dt=rm&dt=ss&dt=t&dt=at&ie=UTF-8&oe=UTF-8&otf=1&ssel=0&tsel=0&tk=' + tk + '&q='+encodeURIComponent(truncStrByWord(newTEXT,100));
...
function SLDetectPATCH(theText){
	...
	GEBI("SL_detect").innerHTML="";
		...
		if(lng!='un'){
			...
			DetLang = "en";
			GEBI("SL_detect").innerHTML = FExtension.element(FExtension.store.get("SL_LOCALIZATION"),'extDetectedEn');
		}
	}, 300);
}
...
function SL_OTHER_PROVIDERS(text,f,t,ind){
 if(SL_TRANS_TEXT=="" || PROV!="Google"){
        if(PROV.toLowerCase()=="yandex") { SL_YANDEX(text,f,t,ind); return false;}
 	var ctrl = "SL_target";
	 ...
	 if(f=="") f=mySourceLang;
		...
		ajaxRequest.onreadystatechange = function(){
			...
			if(ajaxRequest.readyState == 4){
		             ...
		             if(resp.indexOf('<#<')!=-1 || resp.indexOf('>Url Too Long<')!=-1 || resp.indexOf('>Request URL Too Long<')!=-1 || resp.indexOf('"ArgumentOutOfRangeException')!=-1) resp=SL_SETLOC("extlim2000").replace("XXX","4000");
...
function LTR_RTL(ctrl){
	...
	GEBI(ctrl).dir="ltr";
	if(myLang=="ar" || myLang=="iw" || myLang=="fa" || myLang=="ur" || myLang=="yi" || myLang=="ps" || myLang=="sd") GEBI(ctrl).dir="rtl";
}

function MORE_LTR_RTL(){
 	var myLang = DetLang; 	
	GEBI("SL_source").dir="ltr";
	if(myLang=="ar" || myLang=="iw" || myLang=="fa" || myLang=="ur" || myLang=="yi" || myLang=="ps" || myLang=="sd") GEBI("SL_source").dir="rtl";

	myLang = GEBI("SL_langDst").value;
	GEBI("SL_target").dir="ltr";
	if(myLang=="ar" || myLang=="iw" || myLang=="fa" || myLang=="ur" || myLang=="yi" || myLang=="ps" || myLang=="sd") GEBI("SL_target").dir="rtl";

	myLang = DetLang;
	GEBI("SL_back").dir="ltr";
	if(myLang=="ar" || myLang=="iw" || myLang=="fa" || myLang=="ur" || myLang=="yi" || myLang=="ps" || myLang=="sd") GEBI("SL_back").dir="rtl";
}
...
if(CLOSER==0 || localStorage["SL_GotIt"]!="1"){
 ...
 if(box=="SRC") DO_TTS_Detection(text,1);
  setTimeout(function(){
   SL_to = TTSDetLang;
...
function LOCcontrol(st){
    ...
    if(GEBI('SLlocpl').checked == true) GEBI("SLlocbox").src="../../img/util/box-on.png";
...
function SL_YANDEX(text,f,t,ind){
 	 var ctrl = "SL_target";
	 if(ind != 1) ctrl = "SL_back";
