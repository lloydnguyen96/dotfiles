...
function SL_SHOWHIDEPROVIDERS(){
   	if(GEBI("SL_OtherTr").checked!=true) SL_ONE_TR_ONLY(1);
	else SL_ONE_TR_ONLY(0);
}


function PR_BUILDER(st){
...
function SAVE_LIST_PROVIDERS(st){
...
function SAVE_LIST_PROVIDERS_SYN(st1,st2){
	var pset = localStorage[st1].split(",");	
	var items = document.getElementsByTagName("li");
	var OUTlist = "";
	for(var i=0; i<items.length; i++){
            var ID = items[i].id.replace('item-','');
	...
	localStorage[st1] = OUTlist;
	localStorage[st2] = OUTlist;
        PR_BUILDER(st1);
}
